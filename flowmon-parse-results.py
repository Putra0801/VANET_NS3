...
def main(argv):
    ...
    for sim in sim_list:
		...
		txbitrate=txbitrate+flow.txBitrate*1e-3
            if flow.rxBitrate is None:
                print "\tRX bitrate: None"
            else:
                print "\tRX bitrate: %.2f kbit/s" % (flow.rxBitrate*1e-3,)
		rxbitrate=rxbitrate+flow.rxBitrate*1e-3
            if flow.delayMean is None:
                print "\tMean Delay: None"
            else:
                print "\tMean Delay: %.2f ms" % (flow.delayMean*1e3,)
		delay=delay+flow.delayMean*1e3
            if flow.packetLossRatio is None:
                print "\tPacket Loss Ratio: None"
            else:
                print "\tPacket Loss Ratio: %.2f %%" % (flow.packetLossRatio*100)
		PLR=PLR+flow.packetLossRatio*100
	print "\n\nAverage TX bitrate: %.2f kbit/s" % (txbitrate)
	print "Average RX bitrate: %.2f kbit/s" % (rxbitrate)
	print "Average Delay: %.2f ms" % (delay)	
	print "Average Packet Loss Ratio: %.2f %%" % (PLR)
